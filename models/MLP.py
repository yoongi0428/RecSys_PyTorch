...
class MLP(BaseModel):
    def __init__(self, model_conf, num_user, num_item, device):
        ...
        self.hidden_dims = [self.emb_dim] + model_conf.hidden_dims + [1]
        self.hidden_layers = nn.ModuleList([
            nn.Linear(i, o, bias=True)
            for i, o in zip(self.hidden_dims[:-1], self.hidden_dims[1:])
        ])
        self.act = model_conf.activation
    ...
    def forward(self, user_ids, item_ids):
        ...
        user_item_latent = torch.cat((user_latent, item_latent), 1)
        for i, layer in enumerate(self.hidden_layers):
            out = layer(user_item_latent)
            if i + 1 == len(self.hidden_layers):
                out = apply_activation(self.act, out)
            else:
                out = apply_activation('sigmoid', out)
